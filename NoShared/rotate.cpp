...
std::vector<int> RotateShiftHost::push(float angle, int2 shift,
    ...
    int2 subc; // центрированное субполе [-sz0/2..sz0/2-1] (включая точки которые ему не
               // принадлежат)
    int2 sub;  // не центрированное субполе [0..sz0-1] (включая точки которые ему не
    // принадлежат)
...
std::vector<int> RotateShiftHost::pull(float angle, int2 shift,
    ...
    float rad = angle * (3.14159265358979323846f / 180.0f);
    float2 d_rotate = {tanf(rad) * (1.f - sqrtf(2.0f)), sinf(rad * 2.0f) / sqrtf(2.0f)};
